...
![](http://ww1.sinaimg.cn/large/de75fd55gy1g3aiwhbn16j21zc140wqe.jpg)
...
- <http://www.ibm.com/developerworks/cn/linux/l-cn-sshforward/index.html> [![User Link](http://blog.knownsec.com/Knownsec_RD_Checklist/v2.2.html_files/ilink.png)](http://www.ibm.com/developerworks/cn/linux/l-cn-sshforward/index.html)
- 本地转发
...
- Sorted sets，类似 Sets, 但是每个字符串元素都关联到一个叫 *score* 浮动数值（floating number value）。里面的元素总是通过 score 进行着排序，所以不同的是，它是可以检索的一系列元素。（例如你可能会问：给我前面 10 个或者后面 10 个元素）。
...
![](https://ws1.sinaimg.cn/large/de75fd55gy1g35u4fne4fj20ju0jeae1.jpg)



![](https://ws1.sinaimg.cn/large/de75fd55gy1g35uw1f9voj20hi0k2n1h.jpg)
...
+ **社交网络**
    赞/踩、粉丝、共同好友/喜好、推送、下拉刷新等是社交网站的必备功能，由于社交网站访问量通常比较大，而且传统的关系型数据不太适合保存 这种类型的数据，Redis提供的数据结构可以相对比较容易地实现这些功
    能。
+ **消息队列系统**
...
```shell
set key value   		# 添加键值对（覆盖同名 key）
setnx key value 		# key 不存在时设置为 value
mset k1 v1 k2 v2		# 同时设置多个键值对（覆盖）
msetnx k1 v1 k2 v2 		# 所有给定 key 不存在时才设置
setex key seconds value # 将 value 关联到 key，并设置过期时间（秒）
setrange key offset val # ？offset 到底能干啥
get key 				# 获取 key 所对的键值
mget key1 key2 key3 ...	# 获取多个给定 key 的值
getrange key start end  # 返回 key 对应键值的子字符串
getset key value		# 设置 key 键值为 value，并返回 key 的旧键值
setbit key offset value # 
getbit key offset 		# 这两个不知道能干啥
strlen key				# 返回键值长度
incr key				# 创建 key 并自增1，若已存在直接自增（针对数值）
decr key				# 自减
incrby key 3			# 增加 3
decrby key 3			# 减少 3
append key value		# 将 value 拼接到 key 值后面
```

### Hash

```shell
hset key field value	# 将哈希表 key 中的字段 field 设置为 value
hsetnx key field value	# field 不存在时才设置
hmset key f1 v1 f2 v2   # 设置多个字段及相应的值 
hdel key field1 [fie2]  # 删除一个或多个哈希表字段
hexists key field		# 查看指定字段是否存在
hget key field			# 获取存储在哈希表中指定字段的值
hgetall key				# 获取 key 所有字段及值
hincrby key field incr	# 指定字段整数值加 incr
hincrbyfloat key fiel i # 指定字段浮点数值加 i
hkeys key				# 获取所有字段
hvals key 				# 获取所有值
hlen key				# 获取字段数量
```
...
```shell
lpush key v1 [v2]		 # 一个或多个插入列表头部
rpush key v1 [v2] 		 # 插到尾部
rpushx key value		 # 插到尾部，当 key 存在时有效
blpop k1 [k2] timeout 	 # 弹出列表第一个元素，若没有元素则阻塞直到弹出，可指定超时时间
brpop k1 [k2] timeout 	 # 弹出最后一个元素，阻塞
brpoplpush sou des time  # 从 sou 弹出一个值插入 des 中并返回，阻塞
rpoplpush sou des time   # 从 sou 弹出一个值插入 des 中并返回，非阻塞
lindex key index		 # 通过索引获取列表中的元素
linsert key before|after pivot value  # 在指定元素前后插入
llen key				 # 获取列表长度
lpop key				 # 弹出并返回第一个元素
rpop key				 # 弹出并返回最后一个元素
lrange key start stop 	 # 获取指定范围元素
lrem key count value 	 # 移除元素
lset key index value	 # 通过索引设置列表元素
ltrim key start stop     # 只保留指定区间内元素，范围之外的将被删除
```

### Set

```shell
sadd key member1 [memb2]  # 向集合添加一个或多个成员
scard key 				  # 获取集合中的成员数
sdiff key1 [key2]		  # 返回差集
sinter key1 [key2] 		  # 返回交集
sunion key1 [key2]		  # 返回并集
sdiffstore des k1 [k2]    # 返回差集并存到 des 中
sinterstore des k1 [k2]   # 返回交集并存入 des 中
sunionstore des k1 [k2]	  # 返回并集并存入 des 中
sismember key member	  # 判断 member 是否属于 key
smembers key 			  # 返回所有成员
smove sou des member	  # 将 member 从 sou 移到 des
spop key				  # 移除并返回集合中的随机元素
srandmember key [count]	  # 返回一个或多个随机元素
srem key mem1 [mem2]	  # 移除集合中一个或多个成员
```

### Sorted set

在 set 的基础上关联一个 score 来排序

```shell
zadd key score1 member1 [score2] member2  # score 可为浮点数
zscore score tkey
zrange score start stop
